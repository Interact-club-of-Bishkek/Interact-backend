...
REGISTRATION_START = datetime(2026, 1, 12, 0, 0, 0).replace(tzinfo=BISHKEK_TIMEZONE)
REGISTRATION_END = datetime(2026, 1, 13, 0, 0, 0).replace(tzinfo=BISHKEK_TIMEZONE)
...
async def final_submit(message: types.Message, state: FSMContext):
    ...
    if success:
        ...
        await message.answer(
            ...
            "Благодарим Вас за интерес к <b>Interact Club of Bishkek</b> и за время, которое Вы уделили заполнению заявки.\n\n"
            "Ваша анкета передана команде клуба. В ближайшее время мы свяжемся с Вами и расскажем о следующих шагах.\n\n"
            "До скорой встречи и добро пожаловать в сообщество инициативных и неравнодушных людей!",
            parse_mode="HTML"
        )
    else:
        await message.answer(
